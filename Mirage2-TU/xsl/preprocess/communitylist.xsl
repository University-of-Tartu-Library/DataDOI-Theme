...
<xsl:stylesheet
    ...
    <xsl:template match="dri:referenceSet[@id='aspect.artifactbrowser.CommunityBrowser.referenceSet.community-browser']">
        <!-- @max : apply this only on non-front pages -->
        <xsl:choose>
            <xsl:when test="count(//dri:pageMeta/dri:trail) &gt; 1">
                <div id="{@id}" rend="community-browser-wrapper">
                    <xsl:apply-templates mode="community-browser"/>
                </div>
            </xsl:when>
            <xsl:otherwise>
                <div id="{@id}" rend="homepage-community-browse-wrapper row">
                    <xsl:apply-templates mode="homepage-community-browser"/>
                </div>
            </xsl:otherwise>
        </xsl:choose>
    </xsl:template>
    ...
    <xsl:template match="dri:reference" mode="homepage-community-browser">
        ...
        <div>
            <!-- @max : variables and attributes -->
            <xsl:attribute name="rend">
                <xsl:text> col-xxs-12 col-xs-6 col-sm-4 col-md-3 col-lg-5ths image-wrapper</xsl:text>
            </xsl:attribute>
            ...
            <!-- @max relevant code -->
            <div rend="homepage-community-logo">
                <xsl:apply-templates select="document($externalMetadataURL)" mode="homepage-community-browser"/>
            </div>
        </div>
    </xsl:template>
    ...
    <xsl:template match="mets:METS" mode="homepage-community-browser">
    ...
    <xsl:template name="remove-community-slogan">
    </xsl:template>
