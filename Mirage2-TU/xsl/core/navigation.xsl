...
<!--             <xsl:if test="count(/dri:document/dri:meta/dri:pageMeta/dri:metadata[@element='feed']) != 0">
    ...
    <xsl:template match="dri:options/dri:list//dri:list/dri:head" priority="3">
        <!-- @max : we don't want to display stupid "All of Dspace"
        (xmlui.ArtifactBrowser.Navigation.head_all_of_dspace)  message
         -->
        <xsl:if test="not(contains(., 'xmlui.ArtifactBrowser.Navigation.head_all_of_dspace'))">
